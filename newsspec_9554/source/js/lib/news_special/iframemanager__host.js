    ...
    IframeWatcher.prototype = {
        ...
        removeAppWebViewLinksFromHostPage: function () {
            ...
            if (typeof document.querySelector !== 'undefined') {
                appLink = document.querySelector('a[href*="' + window.location.pathname + '"]');
                if (appLink) {
                    appLink.parentNode.removeChild(appLink);
                }
            } else {
                // Support for older browsers
                appLink = document.getElementsByTagName('a');
                for (var idx = 0; idx < appLink.length; ++idx) {
                    if (appLink[idx].href.indexOf(window.location.pathname) >= 0) {
                        appLink[idx].parentNode.removeChild(appLink[idx]);
                    }
                }
            }
        },
